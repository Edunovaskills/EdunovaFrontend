...
export const certificateSchema = yup.object().shape({
  certificatePdf: yup.mixed() // Retain mixed for file handling on frontend
    .test(
      'fileType',
...
export const searchCertificateSchema = yup.object().shape({
  searchKey: yup.string().trim().required('Please enter a certificate key to search').min(5, 'Key must be at least 5 characters long'),
});

export type SearchCertificateSchema = yup.InferType<typeof searchCertificateSchema>;
