...
import { CloudUpload, Delete, Close, Edit, Visibility, Search, TableRows, GridView } from '@mui/icons-material';
...
const PLACEHOLDER_IMAGE_URL = 'https://placehold.co/600x400/CCCCCC/000000?text=No+Image';
...
export const AddBlog: React.FC<AddBlogProps> = () => {
  ...
  } = useAllBlogsForAdminQuery({ page: page + 1, limit: rowsPerPage, search: debouncedSearchQuery });

  // Fetch single blog data when editing
  const {
    data: singleBlogData,
    isLoading: isLoadingSingleBlog,
  } = useBlogByIdAdminQuery(editingBlogId || undefined);
  ...
  const onSubmit = async (data: BlogSchema) => {
    try {
      if (editingBlogId) {
        ...
        showSnackbar({ title: 'Blog updated successfully!', color: 'success' });
      } else {
        // Create new blog
        if (!imageFile) {
          showSnackbar({
        ...
        showSnackbar({ title: 'Blog created successfully!', color: 'success' });
      }
    } catch (error: any) {
      showSnackbar({
  ...
  const handleToggleActive = async (blogId: string, isActive: boolean) => {
    try {
      await updateBlogMutation.mutateAsync({
        blogId: blogId,
        blogPayload: { isActive: isActive },
        imageFile: undefined, // No image update needed for status toggle
      });
      refetchBlogs(); // Re-fetch to show updated status
      showSnackbar({ title: 'Blog status updated successfully!', color: 'success' });
    } catch (error: any) {
      console.error('Failed to toggle blog status:', error);
      showSnackbar({
        title: error?.response?.data?.message || 'Failed to toggle blog status.',
        color: 'error'
      });
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    if (blogToDeleteId) {
      try {
        ...
        showSnackbar({ title: 'Blog deleted successfully!', color: 'success' });
      } catch (error: any) {
        showSnackbar({
          title: error?.response?.data?.message || 'Failed to delete blog.',
          color: 'error',
        });
      } finally {
  ...
  // const totalPages = blogsData?.totalPages || 1; // Not directly used in TablePagination, but useful for custom pagination
  // const currentPage = blogsData?.currentPage || 1; // Not directly used in TablePagination
  ...
  return (
    <Box sx={addBlogStyles.container}>
      ...
      {/* Snackbar alerts for mutations (redundant with useSnackBar, but kept for visibility) */}
      {createBlogMutation.isError && (
        <Alert severity="error" sx={addBlogStyles.alert}>
          {createBlogMutation.error?.response?.data?.message || 'Failed to create blog.'}
        </Alert>
      )}
      {updateBlogMutation.isError && (
        <Alert severity="error" sx={addBlogStyles.alert}>
          {updateBlogMutation.error?.response?.data?.message || 'Failed to update blog.'}
        </Alert>
      )}
      {deleteBlogMutation.isError && (
        <Alert severity="error" sx={addBlogStyles.alert}>
          {deleteBlogMutation.error?.response?.data?.message || 'Failed to delete blog.'}
        </Alert>
      )}

      {/* Success alerts (redundant with useSnackBar, but kept for visibility) */}
      {createBlogMutation.isSuccess && (
        <Alert severity="success" sx={addBlogStyles.alert}>
          {createBlogMutation.data?.message || 'Blog created successfully!'}
        </Alert>
      )}
      {updateBlogMutation.isSuccess && (
        <Alert severity="success" sx={addBlogStyles.alert}>
          {updateBlogMutation.data?.message || 'Blog updated successfully!'}
        </Alert>
      )}
      {deleteBlogMutation.isSuccess && (
        <Alert severity="success" sx={addBlogStyles.alert}>
          {deleteBlogMutation.data?.message || 'Blog deleted successfully!'}
        </Alert>
      )}
          ...
          <Box sx={addBlogStyles.imageUploadContainer}>
            ...
            {preview && (
              <Box sx={addBlogStyles.imagePreviewBox}>
                <img src={preview} alt="Blog Preview" style={addBlogStyles.imagePreview} />
                <IconButton
                  onClick={() => {
                    setPreview(null);
                    setImageFile(null);
      ...
      <Box sx={{ mt: 4 }}>
        ...
        {isLoadingBlogs ? (
          ...
          <Alert severity="error" sx={addBlogStyles.alert}>Error loading blogs: {blogsError?.message}</Alert>
        ) : blogs.length === 0 ? (
          <Alert severity="info" sx={addBlogStyles.alert}>No blogs found.</Alert>
        ) : (
          <>
            {currentView === 'table' ? (
              // Table View
              <TableContainer component={Paper} sx={addBlogStyles.tableContainer}>
                <Table>
                  <TableHead>
                    <TableRow>
                      <TableCell sx={addBlogStyles.tableHeader}>Image</TableCell>
                      <TableCell sx={addBlogStyles.tableHeader}>Title</TableCell>
                      <TableCell sx={addBlogStyles.tableHeader}>Description</TableCell>
                      <TableCell sx={addBlogStyles.tableHeader}>Created By</TableCell>
                      <TableCell sx={addBlogStyles.tableHeader}>Status</TableCell>
                      ...
                      <TableRow key={blog._id} sx={addBlogStyles.tableRow}>
                        <TableCell>
                          <img
                            ...
                            alt={blog.title}
                            style={addBlogStyles.tableImage}
                          />
                        </TableCell>
                        <TableCell sx={addBlogStyles.tableCell}>{blog.title}</TableCell>
                        ...
                        <TableCell sx={addBlogStyles.tableCell}>
                          <Switch
                            ...
                            inputProps={{ 'aria-label': 'blog active status' }}
                          />
                ...
                <TablePagination
                  ...
                  sx={addBlogStyles.paginationWrapper} // Apply wrapper styles here
                />
              </TableContainer>
            ) : (
              // Grid View
              <>
                <Box sx={addBlogStyles.blogGrid}>
                  {blogs.map((blog) => (
                    <Paper key={blog._id} elevation={2} sx={addBlogStyles.blogCard}>
                      <img
                        src={blog.image || PLACEHOLDER_IMAGE_URL}
                        alt={blog.title}
                        style={addBlogStyles.blogCardImage}
                      />
                      <Box sx={addBlogStyles.blogCardContent}>
                        <Typography variant="h6" sx={addBlogStyles.blogCardTitle}>
                          {blog.title}
                        </Typography>
                        <Typography variant="body2" sx={addBlogStyles.blogCardDescription}>
                          {blog.description}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          Created By: {blog.createdBy && typeof blog.createdBy === 'object'
                            ? (blog.createdBy as any).name
                            : 'N/A'}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                        ...
                        <Box sx={addBlogStyles.blogCardActions}>
                          <IconButton color="primary" size="small" onClick={() => handleEditBlog(blog)}>
                            <Edit fontSize="small" />
                          </IconButton>
                          <IconButton color="error" size="small" onClick={() => handleDeleteBlogClick(blog._id)}>
                            <Delete fontSize="small" />
                          </IconButton>
                          <IconButton color="success" size="small" onClick={() => handleViewBlog(blog)}>
                            <Visibility fontSize="small" />
                          </IconButton>
                        </Box>
                      </Box>
