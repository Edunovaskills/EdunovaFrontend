...
export const Badge = styled.span<{ $variant: 'price' | 'level' }>`
...
export const ProgressFill = styled.div<{ $percentage: number }>`
  background: linear-gradient(to right, ${theme.colors.green[500]}, #0d9488);
  height: 100%;
  border-radius: ${theme.borderRadius.full};
...
export const CurriculumIcon = styled.div`
  display: flex;
  align-items: center;
...
export const EnrollButton = styled.button<{ $disabled: boolean }>`
  ...
  background: ${({ $disabled }) => 
    $disabled 
      ...
      : `linear-gradient(135deg, ${theme.colors.green[600]}, #0d9488)`
  };
