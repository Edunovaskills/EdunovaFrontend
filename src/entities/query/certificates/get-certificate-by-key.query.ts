...
import { certificatesApi } from 'entities/api/certificates/certificates.api';
...
export const useDownloadCertificate = () => {
  ...
  const download = async (key: string) => {
    try {
      const blob = await certificatesApi.downloadCertificate(key);
      ...
      show({
        title: (error as any)?.response?.data?.message || 'Failed to download certificate.',
        color: 'error',
      });
    }
  };
