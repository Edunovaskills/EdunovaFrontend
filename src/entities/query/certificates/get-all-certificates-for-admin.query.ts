...
export const useAllCertificatesForAdminQuery = (params: AllCertificatesForAdminQueryParams = {}) => {
  const { page = 1, limit = 10, search = '' } = params;

  return useQuery<CertificateResponse>({
    queryKey: [AllCertificatesForAdminQueryKey, page, limit, search],
    queryFn: async () => {
      const response = await adminCertificateApi.getAllCertificates(page, limit, search);
